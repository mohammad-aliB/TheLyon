...
dispatcher.GetRequest('/Admin/Calendar',function(req,res){
    if(sessionToken==0){
        ...
        var cookies = cookie.parse(req.headers.cookie || '');
        if(cookies.sessionToken==sessionToken){
            calendar.find({}).sort({"date":1}).toArray(function(err, events) {
                if(!err){
                    //console.log(events);
                    //events=[];
    ...
    if(sessionToken==0){
        ...
        if(cookies.sessionToken==sessionToken){
            if(req.postData["CSRF"]=="2222asdfghjklkjhgfdsasdfghjkjhgfdfghgfdfg1232"){
                ...
                articles.update({"ID":req.postData["ID"]},{$set:{"ID":req.postData["ID"],"date":new Date(req.postData["date"]),"author":req.postData["author"],"summary":req.postData["summary"],"published":req.postData["published"],"URL":URL,"body":req.postData["body"],"title":req.postData["title"],"category":req.postData["category"]}},{upsert:true},function(err, result) {
                    if(!err){
                      res.end("successful");
                    }
                });
